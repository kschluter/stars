...
export function canvasArea(elm) {
...
canvasArea.prototype = {
  ...
  get width() {
    return +this.getAttribute('width') || parseInt(getComputedStyle(this).getPropertyValue('width')) || 300
  },
  set width(q) {
    this.style.width = (this.setAttribute('width', q), q) + 'px'
  },
  get height() {
    return +this.getAttribute('height') || parseInt(getComputedStyle(this).getPropertyValue('height')) || 150
  },
  set height(q) {
    this.style.height = (this.setAttribute('height', q), q) + 'px'
  },
  get resizable() {
    return (this._resizable !== undefined && this._resizable) || (this._resizable = ['horizontal', 'vertical', 'both'].indexOf(this.getAttribute('resizable')) + 1)
  },
  set resizable(q) {
  ...
  get resizeActive() {
    return this._resizable && this.cursor.includes('resize')
  },
  get cursor() {
    return this.style.cursor
  },
  set cursor(q) {
    return (this.style.cursor = q)
  },
  get cartesian() {
    return !!eval(this.getAttribute('cartesian'))
  },
  set cartesian(q) {
    this.setAttribute('cartesian', q)
  },
  get view() {
    return this._view
  },
  ...
  zoom: function({ x, y, scl }) {
    ...
    this.notify('view', this._view)
    // stop at 1.0 zoom
    // // console.log(scl, this._view)
    // if (scl < 1 || this._view.scl < 1) {
    //   this._view.x = x + scl * (this._view.x - x)
    //   this._view.y = y + scl * (this._view.y - y)
    //   this._view.scl *= scl
    //   this.notify('view', this._view)
  ...
  pntToUsr: function({ x, y }) {
    let vw = this._view
    return { x: (x - vw.x) / vw.scl, y: (y - vw.y) / vw.scl }
  },
  //    vecToUsr: function({x,y}) { let vw = this._view; return {x:(x - vw.x)/vw.scl, y:(y - vw.y)/vw.scl} },
  resize: function({ width, height }) {
    this.width = width
    this.height = height
    for (let canvases = this.getElementsByTagName('canvas'), i = 0; i < canvases.length; i++) {
      // resize canvas child elements ..
  ...
  handleEvent: function(e) {
    if (e.type in this) this[e.type](this.getEventData(e))
  },
  ...
  mousedown: function(e) {
    ...
    } else this.notify('buttondown', e)
  },
  mouseup: function(e) {
    if (this.resizeActive) {
      window.removeEventListener('mousemove', this.handleEventPtr, false)
      this.addEventListener('mousemove', this, false)
    } else this.notify('buttonup', e)
  },
  mouseenter: function(e) {
    this.notify('pointerenter', e)
  },
  mouseleave: function(e) {
    this.notify('pointerleave', e)
  },

  wheel: function(e) {
    this.zoom({ x: e.x, y: e.y, scl: e.delta > 0 ? 9 / 10 : 10 / 9 })
  },

  resizeMode: function({ x, y }) {
    let mode = this.resizable,
      w = mode && this.width,
      h = mode && this.height,
      cartesian = this.cartesian
    return (
      mode &&
      (mode > 2 && x > w - 3 && ((cartesian && y < 3) || (!cartesian && y > h - 3))
        ? 'nwse'
        : mode % 2 && x > w - 3 ? 'ew' : mode > 1 && ((cartesian && y < 3) || (!cartesian && y > h - 3)) ? 'ns' : false)
    )
  },
  getEventData: function(e) {
    // inconsistent middle button value with 'mouseup' !!
  ...
  notify: function(key, val) {
    let res = false
    if (this.signals[key]) for (let hdl of this.signals[key]) res = res || hdl(val)
    return res
  },
  ...
  remove: function(key, handler) {
    ...
    if (idx >= 0) this.signals[key].splice(idx, 1)
  }
}
...
canvasArea.register = function() {
  const register = () => {
    ...
    for (let elms = document.getElementsByTagName('canvas-area'), i = 0; i < elms.length; i++) canvasArea(Object.setPrototypeOf(elms[i], canvasArea['prototype']))
  }
